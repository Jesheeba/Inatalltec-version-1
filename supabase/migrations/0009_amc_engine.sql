-- ============================================================
-- 0009 — AMC engine (Option C: surgical rename + extend)
--
-- Pre-flight audit (run 2026-05-20 against current db):
--   amc_service_schedule  = 0 rows → safe to drop & recreate
--   amc_contracts          = 0 rows → no value-mapping data work
--   amc_payments           = 0 rows → safe to replace fn_amc_payment_received
--   contract_status column = not present → safe to rename state → contract_status
--
-- Why this file is NOT a single BEGIN/COMMIT:
--   Postgres requires every `ALTER TYPE ... ADD VALUE` to be committed
--   before the new value can be USED (UPDATE row, ::amc_status cast,
--   column default). So phase-1 (enum mutations) runs in autocommit and
--   phase-2 (everything else) is wrapped in a single transaction.
--   Paste the whole file into the Supabase SQL Editor and Run — the
--   editor uses autocommit by default, so each phase-1 statement commits
--   immediately and phase-2's BEGIN/COMMIT bounds the rest.
--
-- Value mapping from legacy uppercase → new lowercase enum:
--   DRAFT                 → draft
--   SIGNED                → pending_payment
--   ACTIVE                → active
--   PENDING_REACTIVATION  → suspended
--   BLOCKED               → suspended
--   RENEWAL_DUE           → active   (renewal-due flag will be tracked
--                                     separately in a later step)
--   EXPIRED               → expired
--   CANCELLED             → cancelled
--
-- UI IMPACT (out of scope for this migration — apply right after):
...
-- ============================================================
-- PHASE 1 — ENUM MUTATIONS (autocommit; do NOT wrap in BEGIN/COMMIT)
-- ============================================================

-- Rename the legacy enum so the column type tracks the new name.
alter type amc_state rename to amc_status;

-- Add the canonical lowercase values. Old uppercase values stay around
-- (Postgres cannot drop enum values); they're dormant after the UPDATE
-- below rewrites every row.
...
-- ============================================================
-- PHASE 2 — REMAINDER (transactional)
-- ============================================================
begin;

-- ============================================================
-- 1) amc_service_status enum (new)
-- ============================================================
do $$
begin
...
-- ============================================================
-- 2) Rename column + map legacy values
-- ============================================================
...
--
--   - Set payment_received_at = now() (the payment's received_at)
...
-- ============================================================
--
-- -- 1. Confirm the column rename + new columns:
...
--
-- -- 2. Confirm the enum got renamed and now has all 15 values
-- --    (8 legacy uppercase + 7 new lowercase):
...
--
-- -- 5. End-to-end smoke: create a contract → pay it → confirm activation +
